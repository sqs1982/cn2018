...
for K=1:10
N=5000;   %input('网络节点个数N:  ');  % 改变节点数量，比较在不同的度情况下，比较不同网络节点数量的扩散情况
%K=6;     %input('输入m:');
p=0.7;
qa=0.4;  %input('接收到A的概率，I变为SA:  '); 改变参数 qa,a以及qb,b,比较在不同的度情况下，比较不同参数情况下， 网络节点扩散情况
a=0.4;    %input('输入转发A的概率');
qb=0.3;    %input('接收到B的概率5，I变为SB:  ');
b=0.7;    %input('输入转发B的概率');
pa=0.2;    %input('接收A信息概率，SB传播B信息结束后成为RAB:  ');
...
K=1:10;
plot(K,xxxxx(K),'-bd');%  扩散结束时A状态的平均密度
hold on
plot(K,yyyyy(K),'-rd');%  扩散结束时B状态的平均密度
hold on
%hold on
%plot(K,mean(SA),'yd-');
%hold on
